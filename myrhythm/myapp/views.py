...
def guitarras(request):
    ...
    guitars = Product.objects.filter(name__icontains='Guitarra')
    total_quantity = 0
    if request.user.is_authenticated:
        total_quantity = sum(item.quantity for item in CartItem.objects.filter(user=request.user))
    
    return render(request, 'guitarras.html', {'guitars': guitars, 'total_quantity': total_quantity})


def baterias(request):
    drums = Product.objects.filter(name__icontains='Batería')
    total_quantity = 0
    if request.user.is_authenticated:
        total_quantity = sum(item.quantity for item in CartItem.objects.filter(user=request.user))
    
    return render(request, 'baterias.html', {'drums': drums, 'total_quantity': total_quantity})
